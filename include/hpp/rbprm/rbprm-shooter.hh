...
typedef boost::shared_ptr<RbPrmShooter> RbPrmShooterPtr_t;
...
class HPP_RBPRM_DLLAPI RbPrmShooter : public core::ConfigurationShooter {
 ...
 public:
  ...
  /// be valid. If filter is empty, no constraints are defined. Otherwise, a configuration can only be valid the all
  /// the ROMS specified in the vector acre colliding with the environment \param normalFilters Specifies constraints
  /// on the accepted normal orientation (in world coordinates) for the reachability condition to be valid. for
  /// instance If a normal filter is set to a normal of  z=(0,0,1) and a range of 0.9, only surfaces with a normal that
 ...
 protected:
  ...
  RbPrmShooter(const pinocchio::RbPrmDevicePtr_t& robot, const core::ObjectStdVector_t& geometries,
               const affMap_t& affordances, const std::vector<std::string>& filter,
               const std::map<std::string, std::vector<std::string> >& affFilters,
               const std::size_t shootLimit = 10000, const std::size_t displacementLimit = 100);
 ...
 private:
  ...
  void randConfigAtPos(const pinocchio::RbPrmDevicePtr_t robot, const std::vector<double>& eulerSo3,

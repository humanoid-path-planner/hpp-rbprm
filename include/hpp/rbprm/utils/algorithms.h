...
/// Implementation of the gift wrapping algorithm to determine the 2D projection
/// of the convex hull of a set of points Dimension can be greater than two, in
/// which case the points will be projected on the z = 0 plane and whether a
/// point belongs to it or not.
///
...
/// ATTENTION: first point is included twice in representation (it is also the
/// last point)
...
/// ATTENTION: first point is included twice in representation (it is also the
/// last point). \param aPoint The point for which to test belonging the the
/// convex hull \return whether aPoint belongs to the convex polygon determined
/// as the convex hull of the rectangle indicated
bool containsHull(T_Point hull, CPointRef aPoint, const double epsilon = 10e-6);

/// Test whether a 2d point belongs to a 2d convex hull determined by a list of
/// unordered points
///
/// \param points list of points
/// \param aPoint The point for which to test belonging the the convex hull
/// \return whether aPoint belongs to the convex polygon determined as the
/// convex hull of the rectangle indicated
bool contains(T_Point points, CPointRef aPoint, const double epsilon = 10e-6);

/// Computes whether two convex polygons intersect
///
/// \param aPointsBegin, aPointsEnd iterators to first and last points of the
/// first polygon \param bPointsBegin, bPointsEnd iterators to first and last
/// points of the second polygon \return the convex polygon resulting from the
/// intersection
T_Point compute2DIntersection(CIT_Point aPointsBegin, CIT_Point aPointsEnd,
                              CIT_Point bPointsBegin, CIT_Point bPointsEnd);
 ...
 * @return distance
 */
double distanceToPlane(const hpp::fcl::Vec3f& n, double t,
                       const hpp::fcl::Vec3f& v);

/**
 * @brief distanceToPlane compute the min distance from a point to an (infinite)
 * plan
...
/**
 * @brief projectPointInsidePlan project a point inside on the given plan,
 * inside the given convex hull
...
/**
 * @brief computeTrianglePlaneDistance compute distance between each vertice of
 * the triangle and a plane
 * @param tri_point
 * @param n plan's normal
 * @param t plan's offset
 ...
 */
void computeTrianglePlaneDistance(hpp::fcl::Vec3f* tri_point,
                                  const hpp::fcl::Vec3f& n, double t,
 ...
 * @return bool
 */
bool insideTriangle(const hpp::fcl::Vec3f& a, const hpp::fcl::Vec3f& b,
                    const hpp::fcl::Vec3f& c, const hpp::fcl::Vec3f& p);
...
/**
 * @brief intersectPolygonePlane Compute the intersection of a polygon and a
 * plane The returned point belongs to the surfaces of @param model2
 * corresponding to the plan equation.
 ...
 * @param useT if false, all the plan with a normal n are used, if true we also
 * check that t is equal
 * @param result
 */
...
// result, bool useT = true, double epsilon = EPSILON);

/**
 * @brief intersectTriangles compute intersection between 2 triangles (not
 * commutative)
 * @param tri first triangle MUST BE DIMENSION 3
 * @param tri2 second triangle (used to compute the plane, the returned point
 * belong to this triangle
 * @param ss
 */
T_Point intersectTriangles(hpp::fcl::Vec3f* tri, hpp::fcl::Vec3f* tri2,
                           std::ostringstream* ss = 0);

/**
 * @brief intersectSegmentPlane compute the intersection between a segment and a
 * plane (infinite)
 * @param s0 first point of the segment
 * @param s1 last point of the segment
 * @param n normal of the plan
 * @param p0 a point in the plan
 * @return A vector of point, empty if no intersection, with one point on the
 * plane if there is an intersection, with s0 and s1 if the segment lie in the
 * plan
 */
T_Point intersectSegmentPlane(Point s0, Point s1, Eigen::Vector3d pn, Point p0);

/**
 * @brief intersectPolygonePlane compute the intersection between a polygone and
 * an (infinite) plane
 * @param polygone
 * @param plane a model which first triangle will define the infinite plan used
 * @param pn : output, normal of the plan
 * @return an ordoned list of point (clockwise), which belong to both the
 * polygone and the plane
 */
T_Point intersectPolygonePlane(BVHModelOBConst_Ptr_t polygone,
...
/**
 * @brief computePlanEquation compute a plan normal and a point in the plan from
 * the first triangle of a BVHModel
 * @param plane

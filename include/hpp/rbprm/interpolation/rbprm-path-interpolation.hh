...
namespace hpp {
namespace rbprm {
typedef std::vector<pinocchio::vector_t,
                    Eigen::aligned_allocator<pinocchio::vector_t> >
    T_Configuration;
...
class HPP_RBPRM_DLLAPI RbPrmInterpolation {
 public:
  ...
  static RbPrmInterpolationPtr_t create(
 ...
 public:
  ...
  /// \param robustnessTreshold minimum value of the static equilibrium
  /// robustness criterion required to accept the configuration (0 by default).
  /// \return a pointer to the created RbPrmInterpolation instance
  rbprm::T_StateFrame Interpolate(
  ...
  /// \param robustnessTreshold minimum value of the static equilibrium
  /// robustness criterion required to accept the configuration (0 by default).
  /// \return The time parametrized list of states according to the reference
  /// path
  rbprm::T_StateFrame Interpolate(
      const affMap_t& affordances,
      const std::map<std::string, std::vector<std::string> >& affFilters,
  ...
  core::Configuration_t configPosition(core::ConfigurationIn_t previous,
                                       const core::PathVectorConstPtr_t path,
                                       double i);

  ///
  /// \brief addGoalConfig add goal configuration (end_state) at the end of a
  /// states list. Modify the last state (or add intermediate states) in the
  /// list to assure that there is only one contact variation between each
  /// states. \param states \return the input list with the last states modified
  /// and the goal state added
  ///
  rbprm::T_StateFrame addGoalConfig(const rbprm::T_StateFrame& states);
  rbprm::T_StateFrame addGoalConfigRec(
 ...
 public:
  ...
  const State start_;
  const State end_;
  bool testReachability_;  // decide if we use the reachability criterion during
                           // interpolation
 ...
 private:
  ...
  void tryReplaceStates(const CIT_StateFrame& from, const CIT_StateFrame to,
                        T_StateFrame& res);
  T_StateFrame trySkipStates(const T_StateFrame& originStates);
  void trySkipStates(const CIT_StateFrame& from, const CIT_StateFrame to,
                     T_StateFrame& res);
  void FilterRepositioning(const CIT_StateFrame& from, const CIT_StateFrame to,
                           T_StateFrame& res);
  T_StateFrame FilterRepositioning(const T_StateFrame& originStates);
  void FilterBreakCreate(const CIT_StateFrame& from, const CIT_StateFrame to,
                         T_StateFrame& res);
  T_StateFrame FilterBreakCreate(const T_StateFrame& originStates);
  StateFrame findBestRepositionState(T_StateFrame candidates,
                                     std::vector<std::string> limbsNames);
  bool testReachability(const State& s0, const State& s1);

 protected:
  RbPrmInterpolation(const core::PathVectorConstPtr_t path,
                     const RbPrmFullBodyPtr_t robot, const State& start,
                     const State& end, const bool testReachability = true,
...
/// - An effector is broken, and the same effector is replaced in the following
/// state \param originStates original state list \param deep if false, only
/// matching configurations in sequence are removed \return A list of key states
/// filtered

...
class HPP_RBPRM_DLLAPI RbPrmLimb {
 public:
  ...
  /// container, used for proximity requests. The Effector is considered as the
  /// last joint of the depth first search on the kinematic subchain.
  ///
  /// \param limb Joint instance that serves as a root for the limb
  /// \param effectorName name of the joint to be considered as the effector of
  /// the limb \param offset position of the effector in joint coordinates
  /// relatively to the effector joint \param unit normal vector of the contact
  /// point, expressed in the effector joint coordinates \param x half width of
  /// the default support polygon of the effector \param y half height of the
  /// default support polygon of the effector \param nbSamples number of samples
  /// to generate for the limb \param evaluate heuristic method used to bias
  /// sample generation \param resolution, resolution of the octree voxels. The
  /// samples generated are stored in an octree data structure to perform
  ...
  /// contactType Whether the contact is a surface contact (orientation matters)
  /// or a punctual contact
  /*static RbPrmLimbPtr_t create (const pinocchio::JointPtr_t limb, const
     fcl::Vec3f &offset, const fcl::Vec3f &normal,const double x, const double
     y, const std::size_t nbSamples, const sampling::heuristic evaluate = 0,
                                const double resolution = 0.1, ContactType
     contactType = _6_DOF);*/
  ...
  ///
  /// \param limb Joint instance that serves as a root for the limb
  /// \param offset position of the effector in joint coordinates relatively to
  /// the effector joint \param limbOffset is the offset between the limb_ joint
  /// position and it's link \param unit normal vector of the contact point,
  /// expressed in the effector joint coordinates \param x half width of the
  /// default support polygon of the effector \param y half height of the
  /// default support polygon of the effector \param nbSamples number of samples
  /// to generate for the limb \param evaluate heuristic method used to bias
  /// sample generation \param resolution, resolution of the octree voxels. The
  /// samples generated are stored in an octree data structure to perform
  ...
  /// problematic in terms of performance. The default value is 3 cm. \param
  /// contactType Whether the contact is a surface contact (orientation matters)
  /// or a punctual contact \param disableEndEffectorCollision Whether the end
  /// effector bodies should be counted for collision detection
  static RbPrmLimbPtr_t create(
      ...
      bool disableEndEffectorCollision = false, bool grasps = false,
 ...
 public:
  ...
  const fcl::Matrix3f
      effectorDefaultRotation_;  // effector transform in rest pose
  const fcl::Vec3f offset_;      // effector location
  const fcl::Vec3f
      limbOffset_;  // offset of between the limb_ joint position and it's link
 ...
 protected:
  ...
  RbPrmLimb(const pinocchio::DevicePtr_t device, std::ifstream& fileStream,
...
HPP_RBPRM_DLLAPI bool saveLimbInfoAndDatabase(const RbPrmLimbPtr_t limb,
                                              std::ofstream& dbFile);

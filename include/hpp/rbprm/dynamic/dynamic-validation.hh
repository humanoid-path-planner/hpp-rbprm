...
class DynamicValidationReport : public core::ValidationReport {
 public:
  DynamicValidationReport(centroidal_dynamics::Vector3 acc) : ValidationReport(), acc_(acc) {}
...
class DynamicValidation : public core::ConfigValidation {
 public:
  static DynamicValidationPtr_t create(bool rectangularContact, double sizeFootX, double sizeFootY, double mass,
                                       double mu, core::DevicePtr_t robot);

  /// Compute whether the configuration is valid
  ///
  /// \param config the config to check for validity,
  /// \retval validationReport report on validation. Must be a valid rbprmReport with the latest collision
  /// informations.

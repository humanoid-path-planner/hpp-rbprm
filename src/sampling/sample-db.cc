...
T_VoxelSample getSamplesPerVoxel(
    const shared_ptr<const octomap::OcTree>& octTree,
...
std::map<std::size_t, fcl::CollisionObject*> generateBoxesFromOctomap(
    const shared_ptr<const octomap::OcTree>& octTree, const fcl::OcTree* tree) {
  ...
  for (std::size_t i = 0; i < boxes_.size(); ++i) {
    ...
    fcl::CollisionObject* obj =
        new fcl::CollisionObject(shared_ptr<fcl::CollisionGeometry>(box),
...
SampleDB::SampleDB(const pinocchio::JointPtr_t limb,
    ...
    : resolution_(resolution),
      ...
      geometry_(shared_ptr<fcl::CollisionGeometry>(octree_)),
...
SampleDB::SampleDB(std::ifstream& myfile, bool loadValues)
    : treeObject_(shared_ptr<CollisionGeometry>(new fcl::Box(1, 1, 1))) {
  ...
  octomapTree_ =
      shared_ptr<const octomap::OcTree>(generateOctree(samples_, resolution_));
  octree_ = new fcl::OcTree(octomapTree_);
  geometry_ = shared_ptr<fcl::CollisionGeometry>(octree_);

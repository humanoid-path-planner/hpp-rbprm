...
double dynamicHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& /*direction*/,
                        const Eigen::Vector3d& /*normal*/, const HeuristicParam& params) {
  fcl::Vec3f effectorPosition =
      transform(sample.effectorPosition_, params.tfWorldRoot_.getTranslation(), params.tfWorldRoot_.getRotation());
...
double EFORTHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction, const Eigen::Vector3d& normal,
...
double EFORTNormalHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                            const Eigen::Vector3d& normal, const HeuristicParam& /*params*/) {
  double EFORT = -direction.transpose() * sample.jacobianProduct_.block<3, 3>(0, 0) * (-direction);
  return EFORT * direction.dot(normal);
}

double ManipulabilityHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& /*direction*/,
                               const Eigen::Vector3d& normal, const HeuristicParam& /*params*/) {
  if (Eigen::Vector3d::UnitZ().dot(normal) < 0.7) return -1;
  return sample.staticValue_ * 10000 * Eigen::Vector3d::UnitZ().dot(normal) * 100000 +
         ((double)rand()) / ((double)(RAND_MAX));
}

double RandomHeuristic(const sampling::Sample& /*sample*/, const Eigen::Vector3d& /*direction*/,
                       const Eigen::Vector3d& /*normal*/, const HeuristicParam& /*params*/) {
  return ((double)rand()) / ((double)(RAND_MAX));
}

double ForwardHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                        const Eigen::Vector3d& normal, const HeuristicParam& /*params*/) {
  ...
  return sample.staticValue_ * 1000. * Eigen::Vector3d::UnitZ().dot(normal) +
         100. * sample.effectorPositionInLimbFrame_.dot(
                    fcl::Vec3f(direction(0), direction(1), sample.effectorPositionInLimbFrame_[2])) +
         ((double)rand()) / ((double)(RAND_MAX));
}

double DynamicWalkHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                            const Eigen::Vector3d& normal, const HeuristicParam& params) {
  ...
  if (direction.norm() == 0 || std::isnan(direction.norm())) {  // test for null vector, functions called before this
                                                                // one can try to normalize direction resulting in NaN
    weightDir = 0;
    dir = fcl::Vec3f(0, 0, 1);
  } else {
    weightDir = 100.;
  ...
  /* hppDout(notice,"eff position = "<<sample.effectorPosition_);
   ...
   ]"); hppDout(notice,"value of dot product = "<<pos.dot(dir)); hppDout(notice,"static value =
   "<<sample.staticValue_);*/

  return weightStatic * sample.staticValue_ + weightDir * pos.dot(dir) +
         1. * pos.dot(fcl::Vec3f(params.comAcceleration_[0], params.comAcceleration_[1], dir[2])) +
         ((double)rand()) / ((double)(RAND_MAX));
}

double StaticHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& /*direction*/,
                       const Eigen::Vector3d& /*normal*/, const HeuristicParam& /*params*/) {
  /*hppDout(info,"sample : ");
...
double fixedStepHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                          const Eigen::Vector3d& normal, const HeuristicParam& params, const double t_step) {
  if (!params.comPath_) {
  ...
  // FIXME : we could factorize all of the above and only do it once for each position of the CoM. But this require to
  // know t_step in contact_generation::generate_contact ...
...
*/
  if ((pSample - pTarget).squaredNorm() > 1)
    hppDout(warning,
            "WARNING : In fixed step heuristic, norm is too high. You should change the hardcoded max value ");
  return (1. - (pSample - pTarget).squaredNorm()) +
         0.1 * sample.staticValue_;  // 1 - because it's an heuristic and not a cost
}

double fixedStep1Heuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                           const Eigen::Vector3d& normal, const HeuristicParam& params) {
  return fixedStepHeuristic(sample, direction, normal, params, 1.);
}

double fixedStep08Heuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                            const Eigen::Vector3d& normal, const HeuristicParam& params) {
  return fixedStepHeuristic(sample, direction, normal, params, 0.8);
}

double fixedStep06Heuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                            const Eigen::Vector3d& normal, const HeuristicParam& params) {
  return fixedStepHeuristic(sample, direction, normal, params, 0.6);
}

double fixedStep04Heuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                            const Eigen::Vector3d& normal, const HeuristicParam& params) {
  return fixedStepHeuristic(sample, direction, normal, params, 0.4);
}

double BackwardHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& direction,
                         const Eigen::Vector3d& normal, const HeuristicParam& /*params*/) {
  return sample.staticValue_ * 10000 * Eigen::Vector3d::UnitZ().dot(normal) -
         100 * sample.effectorPosition_.dot(fcl::Vec3f(direction(0), direction(1), direction(2))) +
         ((double)rand()) / ((double)(RAND_MAX));
}

double DistanceToLimitHeuristic(const sampling::Sample& sample, const Eigen::Vector3d& /*direction*/,
                                const Eigen::Vector3d& /*normal*/, const HeuristicParam& /*params*/) {

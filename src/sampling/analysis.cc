...
struct FullBodyDB {
  ...
  static FullBodyDB& Instance(hpp::pinocchio::DevicePtr_t device, std::size_t nbSamples = 10000) {
    ...
    if (instance_->fullBodyConfigs_.size() < nbSamples) instance_->GenerateFullBodyDB(nbSamples, device);
    return *instance_;
  }
...
double selfCollisionProbability(rbprm::RbPrmFullBodyPtr_t fullBody, const SampleDB& /*sampleDB*/,
...
void distanceRec(const ConfigurationIn_t conf, const std::string& lastJoint, hpp::pinocchio::JointPtr_t currentJoint,
  ...
  if (lastJoint == currentJoint->name() || currentJoint->numberChildJoints() == 0)
    ...
    return distanceRec(conf, lastJoint, currentJoint->childJoint(0), currentDistance);
}
...
double distanceToLimits(rbprm::RbPrmFullBodyPtr_t fullBody, const SampleDB& /*sampleDB*/,
                        const sampling::Sample& sample) {
  // find limb name
...
Configuration_t computeWeightsFromAxis(rbprm::RbPrmLimbPtr_t limb, hpp::pinocchio::DevicePtr_t device) {
  Configuration_t weights =
      Configuration_t::Zero(limb->effector_.joint()->rankInVelocity() - limb->limb_->rankInVelocity() + 1);
  size_t i_weight = 0;
  for (size_type i = limb->limb_->rankInVelocity(); i <= limb->effector_.joint()->rankInVelocity(); ++i) {
    hpp::pinocchio::vector_t jointJacobian =
        device->getJointAtVelocityRank(i)->jacobian().block<6, 1>(0, i).transpose();
 ...
 * @return
 */
double referenceConfiguration(rbprm::RbPrmFullBodyPtr_t fullBody, const SampleDB& /*sampleDB*/,
                              const sampling::Sample& sample) {
  // find limb name
  rbprm::RbPrmLimbPtr_t limb = getLimbFromStartRank(sample.startRank_, fullBody);
  hpp::pinocchio::DevicePtr_t device = fullBody->device_;
  ...
  if (fullBody->postureWeights().size() == fullBody->device_->numberDof()) {
    weights = fullBody->postureWeights().segment(
        limb->limb_->rankInVelocity(), limb->effector_.joint()->rankInVelocity() - limb->limb_->rankInVelocity() + 1);
    ...
    // hppDout(notice,"Analysis : posture weight not defined in fullbody, computed weight :
...
AnalysisFactory::AnalysisFactory(hpp::rbprm::RbPrmFullBodyPtr_t device) : device_(device) {
  ...
  evaluate_.insert(
      std::make_pair("selfCollisionProbability", boost::bind(&selfCollisionProbability, boost::ref(device_), _1, _2)));
  evaluate_.insert(std::make_pair("jointLimitsDistance", boost::bind(&distanceToLimits, boost::ref(device_), _1, _2)));
  evaluate_.insert(
      std::make_pair("ReferenceConfiguration", boost::bind(&referenceConfiguration, boost::ref(device_), _1, _2)));
}

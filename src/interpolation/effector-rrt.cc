...
Transform3f getEffectorTransformAt(core::DevicePtr_t device, const pinocchio::Frame& effector,
...
void getEffectorConfigAt(core::DevicePtr_t device, const pinocchio::Frame& effector, const core::PathPtr_t path,
                         const value_type time, ConfigurationOut_t result) {
...
void getEffectorConfigForConfig(core::DevicePtr_t device, const pinocchio::Frame& effector,
                                const Configuration_t fullBodyConfig, ConfigurationOut_t result) {
...
T_Waypoint getWayPoints(pinocchio::DevicePtr_t device, core::PathPtr_t path, const pinocchio::Frame effector,
  ...
  if (IsLine(res) && effectorDistance > 0.03) {
    ...
    res2.push_back(std::make_pair(1, vector_t::Ones(position->outputSize()) * height + res.front().second +
    ...
    value_type max_height =
        effectorDistance < 0.1 ? 0.03 : std::min(0.07, std::max((value_type)nbWayPoints * 0.01, 0.02));
...
exact_cubic_Ptr splineFromEffectorTraj(RbPrmFullBodyPtr_t fullbody, const pinocchio::Frame effector,
                                       core::PathPtr_t path, const State& startState, const State& nextState,
                                       bool& isLine) {
...
std::vector<pinocchio::JointPtr_t> getJointsByName(RbPrmFullBodyPtr_t fullbody,
 ...
 * @return the path
 */
BezierPathPtr_t buildPredefinedPath(const DevicePtr_t& endEffectorDevice, const Vector3& normal,
  ...
  if (init) {
    ...
    refEffector = BezierPath::create(endEffectorDevice, pts.begin(), pts.end(), config, offsetConfig,
                                     core::interval_t(0., time));
    ...
    refEffector = BezierPath::create(endEffectorDevice, pts.begin(), pts.end(), offsetConfig, config,
                                     core::interval_t(0., time));
...
PathVectorPtr_t computeBezierPath(const DevicePtr_t& endEffectorDevice, const ProblemData& pDataMid,
                                  const EndEffectorPath& endEffPath, value_type timeMid, value_type weightRRT,
                                  const BezierPathPtr_t& refEffectorTakeoff, const BezierPathPtr_t& refEffectorLanding,
                                  bezier_Ptr& refEffectorMidBezier) {
  bezier_com_traj::ResultDataCOMTraj res =
      bezier_com_traj::solveEndEffector<EndEffectorPath>(pDataMid, endEffPath, timeMid, weightRRT);
  if (!res.success_) {
  ...
  BezierPathPtr_t refEffectorMid =
      BezierPath::create(endEffectorDevice, refEffectorMidBezier, refEffectorTakeoff->end(),
                         refEffectorLanding->initial(), core::interval_t(0., timeMid));

  // merge the 3 curves :
  PathVectorPtr_t refEffectorPath =
      PathVector::create(refEffectorMid->outputSize(), refEffectorMid->outputDerivativeSize());
...
void computePredefConstants(double /*dist_translation*/, double p_max, double /*p_min*/, double t_total,
                            double& t_predef, double& posOffset, double& /*velOffset*/, double& /*a_max_predefined*/) {
  double timeMid = (t_total - (2 * t_predef)) / 2.;
  posOffset = p_max / (1. + 4. * timeMid / t_predef + 6. * timeMid * timeMid / (t_predef * t_predef) -
...
core::PathPtr_t generateEndEffectorBezier(RbPrmFullBodyPtr_t fullbody, core::ProblemSolverPtr_t problemSolver,
  ...
  const value_type timeDelay = 0.05;  // this is the time during the 'single support' phase where the feet don't move.
                                      // It is needed to allow a safe mass transfer without exiting the flexibility.
  ...
  computePredefConstants(dist_translation, p_max, p_min, totalTime, timeTakeoff, posOffset, velOffset,
                         a_max_predefined);
  // velOffset = 0.;
  // a_max_predefined = 0.;
  ...
  BezierPathPtr_t refEffectorTakeoff =
      buildPredefinedPath(endEffectorDevice, startNormal, initConfig, posOffset, velOffset, timeTakeoff, true,
                          takeoffConfig, pDataTakeoff, a_max_predefined);
  BezierPathPtr_t refEffectorLanding =
      buildPredefinedPath(endEffectorDevice, nextNormal, endConfig, posOffset, -velOffset, timeLanding, false,
                          landingConfig, pDataLanding, a_max_predefined);
  ...
  bezier_Ptr refEffectorMidBezier;
  PathVectorPtr_t refEffectorPath;
  refEffectorPath = computeBezierPath(endEffectorDevice, pDataMid, endEffPath, timeMid, 0., refEffectorTakeoff,
  ...
  if (!refEffectorPath) {
    ...
    // FIXME : using pathId = problemSolver->paths().size()  this way assume that the path returned by this method will
    // be the next added in problemSolver. As there is no access to problemSolver here, it's the best workaround.
    return refEffectorPath;
  }
}
...
std::vector<core::PathVectorPtr_t> fitBeziersToPath(RbPrmFullBodyPtr_t fullbody, const pinocchio::Frame& effector,
  ...
  core::PathVectorPtr_t fullBodyPathVector =
      core::PathVector::create(fullBodyComPath->outputSize(), fullBodyComPath->outputDerivativeSize());
  ...
  */
  ...
  bezier_com_traj::ProblemData pDataLanding, pDataTakeoff;
  BezierPathPtr_t refEffectorTakeoff =
      buildPredefinedPath(endEffectorDevice, startState.contactNormals_.at(effectorName), initConfig, posOffset,
                          velOffset, timeTakeoff, true, takeoffConfig, pDataTakeoff, a_max_predefined);
  BezierPathPtr_t refEffectorLanding =
      buildPredefinedPath(endEffectorDevice, nextState.contactNormals_.at(effectorName), endConfig, posOffset,
                          -velOffset, timeLanding, false, landingConfig, pDataLanding, a_max_predefined);
  ...
  for (std::vector<value_type>::const_iterator it_weight = weightRRT.begin(); it_weight != weightRRT.end();
       ++it_weight) {
    hppDout(notice, "Compute bezier path for weight : " << *it_weight);
    bezierPath = computeBezierPath(endEffectorDevice, pDataMid, endEffPath, timeMid, (*it_weight), refEffectorTakeoff,
                                   refEffectorLanding, refEffectorMidBezier);
    if (bezierPath) {
  ...
  pDataMid.constraints_.flag_ =
      INIT_POS | INIT_VEL | INIT_ACC | END_ACC | END_VEL | END_POS | INIT_JERK | END_JERK | THREE_FREE_VAR;

  for (std::vector<value_type>::const_iterator it_weight = weightRRT.begin() + 1; it_weight != weightRRT.end();
       ++it_weight) {
    hppDout(notice, "Compute bezier path for weight : " << *it_weight);
    bezierPath = computeBezierPath(endEffectorDevice, pDataMid, endEffPath, timeMid, (*it_weight), refEffectorTakeoff,
                                   refEffectorLanding, refEffectorMidBezier);
...
core::PathPtr_t effectorRRTFromPath(RbPrmFullBodyPtr_t fullbody, core::ProblemSolverPtr_t problemSolver,
                                    const PathPtr_t comPath, const PathPtr_t fullBodyComPath, const State& startState,
                                    const State& nextState, const std::size_t /*numOptimizations*/,
                                    const bool keepExtraDof, const PathPtr_t refPath,
  ...
  std::vector<PathVectorPtr_t> listPathBezier =
      fitBeziersToPath(fullbody, effector, comPath->length(), fullBodyComPath, startState, nextState);
  ...
  for (std::vector<PathVectorPtr_t>::const_iterator it_path = listPathBezier.begin();
    ...
    SetEffectorRRTConstraints constraintFactory(comPath, *it_path, refPath, effector, endEffectorDevice,
    ...
    try {
      interpolatedPath =
          interpolateStatesFromPath<EffectorRRTHelper, EffectorRRTShooterFactory, SetEffectorRRTConstraints>(
              ...
              /*numOptimizations this should be different from the numOptimization used by comRRT*/ 1, keepExtraDof,
              0.001, maxIterationRRT);
      if (interpolatedPath) {
        success_rrt = true;
...
core::PathPtr_t effectorRRTFromPath(RbPrmFullBodyPtr_t fullbody, core::ProblemSolverPtr_t problemSolver,
  ...
  core::PathPtr_t fullBodyComPath =
      comRRT(fullbody, problemSolver, comPath, startState, nextState, numOptimizations, true);
  core::PathVectorPtr_t fullBodyPathVector =
      core::PathVector::create(fullBodyComPath->outputSize(), fullBodyComPath->outputDerivativeSize());
  fullBodyPathVector->appendPath(fullBodyComPath);
  ...
  return effectorRRTFromPath(fullbody, problemSolver, comPath, fullBodyComPath, startState, nextState,
                             numOptimizations, keepExtraDof, refPath, constrainedJointPos, constrainedLockedJoints);
}
...
core::PathPtr_t effectorRRT(RbPrmFullBodyPtr_t fullbody, ProblemSolverPtr_t problemSolver, const PathPtr_t comPath,
  ...
  return effectorRRTFromPath(fullbody, problemSolver, comPath, startState, nextState, numOptimizations, keepExtraDof,
...
core::PathPtr_t effectorRRT(RbPrmFullBodyPtr_t fullbody, core::ProblemSolverPtr_t problemSolver,
                            ...
                            const std::vector<std::string>& constrainedLockedJoints) {
  return effectorRRTFromPath(fullbody, problemSolver, comPath, startState, nextState, numOptimizations, keepExtraDof,
                             core::PathPtr_t(), constrainedJointPos, constrainedLockedJoints);
}
  ...
  if (refFullbody_) {
    ...
    for (std::vector<pinocchio::JointPtr_t>::const_iterator cit = constrainedJointPos_.begin();
      ...
      Create6DEffectorConstraint<EffectorRRTHelper, core::PathPtr_t>(
          helper, refFullbody_, helper.fullBodyDevice_->getFrameByName((*cit)->name()));
    }
  }

...
RbPrmInterpolationPtr_t RbPrmInterpolation::create(const hpp::rbprm::RbPrmFullBodyPtr_t robot,
                                                   ...
                                                   const core::PathVectorConstPtr_t path, const bool testReachability,
...
core::Configuration_t RbPrmInterpolation::configPosition(core::ConfigurationIn_t previous,
                                                         const core::PathVectorConstPtr_t path, double i) {
...
///
/// \brief replaceLimbContactForState Try to create a state with all the contacts of stateCurrent, except for LimbId
/// which have the contact of stateGoal \param robot \param stateCurrent \param stateGoal \param limbId \return a
/// projection report
///
projection::ProjectionReport replaceLimbContactForState(RbPrmFullBodyPtr_t robot, State stateCurrent, State stateGoal,
                                                        std::string limbId) {
  ...
  interpolation::addContactConstraints(robot, robot->device_, proj, stateCurrent,
                                       stateCurrent.fixedContacts(stateCurrent));
  std::vector<bool> rotationMaskGoal;
  for (std::size_t i = 0; i < 3; ++i) {
    rotationMaskGoal.push_back(true);
  }
  return projection::projectEffector(proj, robot, limbId, robot->GetLimb(limbId), robot->GetCollisionValidation(),
                                     configuration, stateGoal.contactRotation_.at(limbId), rotationMaskGoal,
                                     stateGoal.contactPositions_.at(limbId), stateGoal.contactNormals_.at(limbId),
                                     stateCurrent);
}

// greedy algorithm to try the combination of order to move the legs until a successfull one is found
rbprm::T_StateFrame RbPrmInterpolation::addGoalConfigRec(const rbprm::T_StateFrame& states,
  ...
  for (size_t id = 0; id < variations.size(); ++id) {
    ...
    if (projReport.success_) {
      ...
      results.push_back(std::make_pair(states.back().first,
                                       projReport.result_));  // there will be 2 states at the same time index ...
...
// Try to add the desired final configuration at the end of the current contact sequence, while guaranteing that there
// is always only one contact variation between states
rbprm::T_StateFrame RbPrmInterpolation::addGoalConfig(const rbprm::T_StateFrame& states) {
  ...
  std::vector<std::string> variationsGoal(
      lastState.contactVariations(end_));  // all limb that must be moved to reach the goal configuration
  ...
  robot_->usePosturalTaskContactCreation(
      false);  // temporary disable this setting for projecting exactly on the goal config
  // Last state and end are not adjacent, we keep lastState in the list and produce intermediate states for each
  // contact transition
 ...
 * @return
 */
core::Configuration_t loadPreviousConfiguration(const DevicePtr_t& device, const core::Configuration_t& previous,
...
rbprm::T_StateFrame RbPrmInterpolation::Interpolate(const affMap_t& affordances,
                                                    const std::map<std::string, std::vector<std::string> >& affFilters,
                                                    const hpp::rbprm::T_Configuration& configs,
                                                    const double robustnessTreshold,
                                                    const pinocchio::value_type timeStep,
                                                    const pinocchio::value_type initValue, const bool filterStates) {
  hppDout(notice, "Begin interpolate in path-interpolation, number of configs to test : " << configs.size());
  int nbFailures = 0;
  size_t accIndex = robot_->device_->configSize() - robot_->device_->extraConfigSpace().dimension() +
                    3;  // index of the start of the acceleration vector (of size 3), in the configuration vector
  ...
  for (CIT_Configuration cit = configs.begin() + 1; cit != configs.end(); ++cit, currentVal += timeStep) {
    ...
    hpp::rbprm::contact::ContactReport rep =
        contact::ComputeContacts(previous, robot_, configuration, affordances, affFilters, direction,
                                 robustnessTreshold, acc, comPath, currentVal, testReachability_, quasiStatic_);
    ...
    hppDout(notice, "success : " << rep.success_ << " ; contacts maintained  : " << rep.contactMaintained_
                                 << " ; contact created : " << rep.contactCreated_
                                 << " ; sameAsPrevious : " << sameAsPrevious << " ; multipleBreak : " << multipleBreaks
                                 << " ; repositionned : " << respositioned << " ; allowFailure : " << allowFailure
                                 << " ; status :" << rep.status_);
    ...
    r(["<<pinocchio::displayConfig(states.back().second.configuration_)<<"])"); hppDout(notice,"First configuration for
    state "<<states.size()<<" : r(["<<pinocchio::displayConfig(newState.configuration_)<<"])");
    }
    ...
    */
    ...
    if (!sameAsPrevious) {
      states.push_back(std::make_pair(currentVal, newState));
      hppDout(notice, "new state added at index " << states.size() - 1 << " conf = r(["
                                                  << pinocchio::displayConfig(states.back().second.configuration_)
                                                  << "])");
      lastIterator = cit;
    } else {
      hppDout(notice,
              "Same as previous, new config = r([" << pinocchio::displayConfig(newState.configuration_) << "])");
    }
...
void RbPrmInterpolation::init(const RbPrmInterpolationWkPtr_t& weakPtr) { weakPtr_ = weakPtr; }
...
StateFrame RbPrmInterpolation::findBestRepositionState(T_StateFrame candidates, std::vector<std::string> limbsNames) {
  ...
  double currentScore;
  fcl::Vec3f direction =
      candidates.back().second.configuration_.head<3>() - candidates.front().second.configuration_.head<3>();
  // TODO : iterate over candidates and call the heuristic for limbsName. Then return the state with the best score
  // (average of scores if more than one limbs)
  for (CIT_StateFrame sit = candidates.begin(); sit != candidates.end(); ++sit) {
    ...
    for (std::vector<std::string>::const_iterator lit = limbsNames.begin(); lit != limbsNames.end(); ++lit) {
      ...
      sampling::Sample sample(limb->limb_, limb->effector_, sit->second.configuration_, limb->offset_,
                              limb->limbOffset_, 0);
      currentScore +=
          limb->evaluate_(sample, direction, sit->second.contactNormals_.at(*lit), sampling::HeuristicParam());
    }
...
void RbPrmInterpolation::FilterRepositioning(const CIT_StateFrame& from, const CIT_StateFrame to, T_StateFrame& res) {
  ...
  if (EqStringVec(current.contactBreaks(current_m1), current_p1.contactBreaks(current_m1)) &&
      EqStringVec(current.contactCreations(current_m1), current_p1.contactCreations(current)) &&
      testReachability(current_m1, current_p1)) {
    if (from + 1 == to) return;
    // Check if there is others state with the same contacts, and only add the one with the best score for the
    // heuristic :
    ...
    // iterate over respoitionnedStates and find the one with the best score with the heuristic (for the limb that
    move)
...
void RbPrmInterpolation::FilterBreakCreate(const CIT_StateFrame& from, const CIT_StateFrame to, T_StateFrame& res) {
  ...
  if (current.contactCreations(current_m1).empty() && current_p1.contactBreaks(current).empty() &&
      EqStringVec(current_p1.contactCreations(current), current.contactBreaks(current_m1)) &&
      testReachability(current_m1, current_p1)) {
    if (from + 1 == to) return;
    res.push_back(std::make_pair((from + 1)->first, (from + 1)->second));
...
T_StateFrame FilterObsolete(const T_StateFrame& originStates) {
  ...
  for (CIT_StateFrame cit2 = originStates.begin() + 1; cit2 != originStates.end() - 1; ++cit, ++cit2) {
...
void RbPrmInterpolation::tryReplaceStates(const CIT_StateFrame& from, const CIT_StateFrame to, T_StateFrame& res) {
  ...
  if (ci3.contactCreations(ci2).size() == 1 && EqStringVec(ci1.contactBreaks(ci0), ci3.contactBreaks(ci2)) &&
    ...
    projection::ProjectionReport rep =
        projection::projectStateToObstacle(robot_, contactCreate, robot_->GetLimb(contactCreate), s1_bis, n, p);
    ...
    if ((p1 - p).norm() < 0.2 && rep.success_ &&
        robot_->GetCollisionValidation()->validate(rep.result_.configuration_, rport) &&
...
void RbPrmInterpolation::trySkipStates(const CIT_StateFrame& from, const CIT_StateFrame to, T_StateFrame& res) {
  ...
  if (ci2.contactCreations(ci1).size() == 1 && EqStringVec(ci1.contactBreaks(ci0), ci3.contactBreaks(ci2)) &&
      EqStringVec(ci1.contactBreaks(ci0), ci1.contactCreations(ci0)) &&
      EqStringVec(ci1.contactBreaks(ci0), ci3.contactCreations(ci2)) &&
      !EqStringVec(ci1.contactBreaks(ci0), ci2.contactBreaks(ci1)) &&
      !EqStringVec(ci1.contactCreations(ci0), ci2.contactCreations(ci1))) {
    ...
    // fcl::Matrix3f r = ci3.contactRotation_.at(contactCreate);
    projection::ProjectionReport rep =
        projection::projectStateToObstacle(robot_, contactCreate, robot_->GetLimb(contactCreate), s2_bis, n, p);
    hppDout(notice, "projection success : " << rep.success_);
    if (rep.success_) {
...
T_StateFrame RbPrmInterpolation::FilterStates(const T_StateFrame& originStates, const bool deep) {
  ...
  std::size_t idx = 0;
  for (T_StateFrame::const_iterator cit2 = originStates.begin() + 1; cit2 != originStates.end() - 1;
       ++cit, ++cit2, ++idx) {
    const State prev = cit->second;
    const State next = cit2->second;
    std::vector<std::string> breaks = next.contactBreaks(prev);
  ...
  hppDout(notice, "End of checks for too many contact changes, filter redunbdant states, size of list :  "
                      << originStates.size());

...
hpp::rbprm::T_RomValidation createRomValidations(const hpp::pinocchio::RbPrmDevicePtr_t& robot,
  ...
  for (hpp::pinocchio::T_Rom::const_iterator cit = robot->robotRoms_.begin(); cit != robot->robotRoms_.end(); ++cit) {
    ...
    } else {  // no affordance filter defined for this ROM, assume it can collide with all the objects
...
RbPrmValidationPtr_t RbPrmValidation::create(const pinocchio::RbPrmDevicePtr_t& robot,
                                             const std::vector<std::string>& filter,
                                             const std::map<std::string, std::vector<std::string> >& affFilters,
                                             const hpp::rbprm::affMap_t& affordances,
                                             const core::ObjectStdVector_t& geometries) {
  RbPrmValidation* ptr = new RbPrmValidation(robot, filter, affFilters, affordances, geometries);
  return RbPrmValidationPtr_t(ptr);
}

RbPrmValidation::RbPrmValidation(const pinocchio::RbPrmDevicePtr_t& robot, const std::vector<std::string>& filter,
                                 const std::map<std::string, std::vector<std::string> >& affFilters,
                                 const hpp::rbprm::affMap_t& affordances,
  ...
  for (std::vector<std::string>::const_iterator cit = filter.begin(); cit != filter.end(); ++cit) {
...
bool RbPrmValidation::validate(const Configuration_t& config, ValidationReportPtr_t& validationReport) {
  return validate(config, validationReport, defaultFilter_);
}

bool RbPrmValidation::validate(const Configuration_t& config, const std::vector<std::string>& filter) {
...
bool RbPrmValidation::validate(const Configuration_t& config, hpp::core::ValidationReportPtr_t& validationReport,
  ...
  success =
      validateRoms(config, filter, rbprmReport) && success && boundValidation_->validate(config, validationReport);
...
bool RbPrmValidation::validateTrunk(const Configuration_t& config,
  ...
  return trunkValidation_->validate(config,
                                    validationReport);  // && boundValidation_->validate(config,validationReport);
}

void RbPrmValidation::addObstacle(const CollisionObjectConstPtr_t& object) { trunkValidation_->addObstacle(object); }

void RbPrmValidation::removeObstacleFromJoint(const JointPtr_t& joint, const CollisionObjectPtr_t& obstacle) {
  trunkValidation_->removeObstacleFromJoint(joint, obstacle);
  for (T_RomValidation::const_iterator cit = romValidations_.begin(); cit != romValidations_.end(); ++cit) {

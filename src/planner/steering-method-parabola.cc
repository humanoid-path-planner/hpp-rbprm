...
core::PathPtr_t SteeringMethodParabola::impl_compute(core::ConfigurationIn_t q1, core::ConfigurationIn_t q2) const {
  hppDout(info, "q_init: " << hpp::pinocchio::displayConfig(q1));
  hppDout(info, "q_goal: " << hpp::pinocchio::displayConfig(q2));
  hppDout(info, "g_: " << g_ << " , mu_: " << mu_ << " , V0max: " << V0max_ << " , Vimpmax: " << Vimpmax_);
...
core::PathPtr_t SteeringMethodParabola::compute_3D_path(core::ConfigurationIn_t q1, core::ConfigurationIn_t q2) const {
  ...
  const size_type index = device_.lock()->configSize() - device_.lock()->extraConfigSpace().dimension();  // ecs index
...
*/  // Commented in order to remove non friction test (testing)
  ...
  bool fail3 =
      third_constraint(fail, X_theta, Z, alpha_imp_min, alpha_imp_max, &alpha_imp_sup, &alpha_imp_inf, n2_angle);
  ...
  if (n2_angle > 0) {
    ...
    alpha_inf_bound = std::max(std::max(alpha_imp_inf, alpha_lim_minus), std::max(alpha_0_min, alpha_inf4 + Dalpha_));

    if (alpha_imp_min < -M_PI / 2) {
      alpha_lim_plus = std::min(alpha_lim_plus, alpha_imp_plus);
      alpha_sup_bound = std::min(alpha_0_max, std::min(alpha_lim_plus, M_PI / 2));
    } else {  // alpha_imp_sup is worth
      alpha_lim_plus = std::min(alpha_lim_plus, alpha_imp_plus);
      alpha_sup_bound = std::min(std::min(alpha_0_max, M_PI / 2), std::min(alpha_lim_plus, alpha_imp_sup));
    }
  } else {  // down-oriented cone
    if (alpha_imp_max < M_PI / 2) {
      alpha_lim_minus = std::max(alpha_lim_minus, alpha_imp_minus);
      alpha_inf_bound =
          std::max(std::max(alpha_imp_inf, alpha_lim_minus), std::max(alpha_0_min, alpha_inf4 + Dalpha_));
    } else {  // alpha_imp_max >= M_PI/2 so alpha_imp_inf inaccurate
      alpha_lim_minus = std::max(alpha_lim_minus, alpha_imp_minus);
      alpha_inf_bound = std::max(std::max(alpha_0_min, alpha_inf4 + Dalpha_), alpha_lim_minus);
    }
    alpha_lim_plus = std::min(alpha_lim_plus, alpha_imp_plus);
    alpha_sup_bound = std::min(std::min(alpha_0_max, M_PI / 2), std::min(alpha_lim_plus, alpha_imp_sup));
  }
  ...
  const vector_t coefsInf = computeCoefficients(alpha_inf_bound, theta, X_theta, Z, x_theta_0, z_0);
  bool maxHeightRespected = parabMaxHeightRespected(coefsInf, x_theta_0, x_theta_imp);
  ...
  vector_t coefs = computeCoefficients(alpha, theta, X_theta, Z, x_theta_0, z_0);
  ...
  ParabolaPathPtr_t pp = ParabolaPath::create(device_.lock(), q1, q2, computeLength(q1, q2, coefs), coefs, V0_, Vimp_,
  ...
  if (hasCollisions || !maxHeightRespected) {
    ...
    while ((hasCollisions || !maxHeightRespected) && n < nLimit_) {
      ...
      maxHeightRespected = parabMaxHeightRespected(coefs, x_theta_0, x_theta_imp);
      pp = ParabolaPath::create(device_.lock(), q1, q2, computeLength(q1, q2, coefs), coefs, V0_, Vimp_,
                                initialROMnames_, endROMnames_);
...
core::PathPtr_t SteeringMethodParabola::compute_random_3D_path(core::ConfigurationIn_t q1, core::ConfigurationIn_t q2,
  ...
  core::PathPtr_t validPart;
  /* Define some constants */
  // const core::size_type index = device_.lock ()->configSize() - device_.lock ()->extraConfigSpace ().dimension ();
  // // ecs index
  const value_type x_0 = q1(0);
  const value_type y_0 = q1(1);
  const value_type z_0 = q1(2);
  ...
  if (alpha_1_minus_ < 0) alpha_1_minus_ = 0;  // otherwise we go in the wrong direction
  value_type interval =
      (alpha_1_plus_ - alpha_1_minus_) / 2.;  // according to friction cone computed in compute_3d_path
  ...
  *v0 = v;
  ...
  const value_type X_theta = X * cos(theta) + Y * sin(theta);
#ifdef HPP_DEBUG
  const value_type x_theta_0_dot = sqrt((g_ * X_theta * X_theta) / (2 * (X_theta * tan(alpha) - Z)));
  ...
  const value_type Vimp =
      sqrt(1 + (-g_ * X * inv_x_th_dot_0_sq + tan(alpha)) * (-g_ * X * inv_x_th_dot_0_sq + tan(alpha))) *
  ...
  // parabola path with alpha_0 as the middle of alpha_0 bounds
  ParabolaPathPtr_t pp = ParabolaPath::create(device_.lock(), q1, *qnew, computeLength(q1, *qnew, coefs), coefs);
  bool hasCollisions = !rbPathValidation->validate(pp, false, validPart, report, filter);
  bool maxHeightRespected = parabMaxHeightRespected(coefs, x_theta_0, x_theta_imp);
...
bool SteeringMethodParabola::second_constraint(const value_type &X, const value_type &Y, value_type *alpha_lim_plus,
...
bool SteeringMethodParabola::third_constraint(bool fail, const value_type &X, const value_type &Y,
...
bool SteeringMethodParabola::fiveth_constraint(const core::ConfigurationIn_t q, const value_type theta,
                                               const int /*number*/, value_type *delta) const {
  const size_type index = device_.lock()->configSize() - device_.lock()->extraConfigSpace().dimension();
  ...
  const bool nonVerticalCone =
      (psi < -phi && psi >= -M_PI / 2) || (psi > phi && psi < M_PI - phi) || (psi > M_PI + phi && psi <= 3 * M_PI / 2);
  ...
  if (denomK > -1e-6 && denomK < 1e-6) {  // denomK (or 'A') = 0
    ...
    if (tanThetaDef) {
      ...
      if (U + V * tanTheta != 0) {
        const value_type numH = mu_ * mu_ * (U * U + V * V * tanTheta * tanTheta) - U * U * tanTheta * tanTheta -
                                V * V + 2 * U * V * tanTheta * (1 + mu_ * mu_) - W * W * (1 + tanTheta * tanTheta);
  ...
  if (tanThetaDef) {
    ...
    const value_type tantheta = tan(theta);
    value_type discr = (U * U + W * W) * mu_ * mu_ - V * V - U * U * tantheta * tantheta +
                       (V * V + W * W) * mu_ * mu_ * tantheta * tantheta + 2 * (1 + mu_ * mu_) * U * V * tantheta;
    ...
    const value_type K1 =
        (sqrt(discr) + U * W + U * W * mu_ * mu_ + V * W * tantheta + V * W * mu_ * mu_ * tantheta) / denomK;
    const value_type K2 =
        (-sqrt(discr) + U * W + U * W * mu_ * mu_ + V * W * tantheta + V * W * mu_ * mu_ * tantheta) / denomK;
    ...
    value_type cos2delta = epsilon * (1 + tantheta * tantheta + K1 * K2) /
                           sqrt((1 + tantheta * tantheta + K1 * K1) * (1 + tantheta * tantheta + K2 * K2));
    hppDout(info, "cos(2*delta): " << cos2delta);
    *delta = 0.5 * acos(cos2delta);
    hppDout(info, "delta: " << *delta);
    assert(*delta <= phi + 1e-5);
...
bool SteeringMethodParabola::sixth_constraint(const value_type &X, const value_type &Y, value_type *alpha_imp_plus,
...
value_type SteeringMethodParabola::lengthFunction(const value_type x, const vector_t coefs) const {
  const value_type y = sqrt(1 + (2 * coefs(0) * x + coefs(1)) * (2 * coefs(0) * x + coefs(1)));
  return y;
}
...
// test (pierre) :
value_type SteeringMethodParabola::computeLength(const core::ConfigurationIn_t q1, const core::ConfigurationIn_t q2,
                                                 const vector_t coefs) const {
  const value_type theta = coefs(3);
...
vector_t SteeringMethodParabola::computeCoefficients(const value_type alpha, const value_type theta,
  ...
  coefs(2) = z_0 - tan(alpha) * x_theta_0 - 0.5 * g_ * x_theta_0 * x_theta_0 * inv_x_th_dot_0_sq;
  coefs(3) = theta;
  coefs(4) = alpha;
...
#ifdef HPP_DEBUG
  const value_type Vimp =
      sqrt(1 + (-g_ * X_theta * inv_x_th_dot_0_sq + tan(alpha)) * (-g_ * X_theta * inv_x_th_dot_0_sq + tan(alpha))) *
      x_theta_0_dot;
#endif
  hppDout(info, "V0: " << V0);
...
bool SteeringMethodParabola::parabMaxHeightRespected(const vector_t coefs, const value_type x_theta_0,
                                                     const value_type x_theta_imp) const {
  const value_type x_theta_max = -0.5 * coefs(1) / coefs(0);
  const value_type z_x_theta_max = coefs(0) * x_theta_max * x_theta_max + coefs(1) * x_theta_max + coefs(2);
...
value_type SteeringMethodParabola::dichotomy(value_type a_inf, value_type a_plus, std::size_t n) const {

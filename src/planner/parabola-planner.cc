...
ParabolaPlannerPtr_t ParabolaPlanner::createWithRoadmap(const core::Problem& problem,
...
ParabolaPlanner::ParabolaPlanner(const core::Problem& problem, const core::RoadmapPtr_t& roadmap)
...
bool belongs(const core::ConfigurationPtr_t& q, const core::Nodes_t& nodes) {
  for (core::Nodes_t::const_iterator itNode = nodes.begin(); itNode != nodes.end(); ++itNode) {
...
void ParabolaPlanner::startSolve() {
  ...
  for (core::Configurations_t::const_iterator itGoal = goals.begin(); itGoal != goals.end(); ++itGoal) {
...
core::PathPtr_t ParabolaPlanner::extend(const core::NodePtr_t& near, const core::ConfigurationPtr_t& target) {
...
core::PathPtr_t ParabolaPlanner::extendParabola(const core::NodePtr_t& near, const core::ConfigurationPtr_t& target) {
  const core::SteeringMethodPtr_t& sm(problem().steeringMethod());
  const core::ConstraintSetPtr_t& constraints(sm->constraints());
  core::PathPtr_t path;
  if (constraints) {
...
void ParabolaPlanner::oneStep() {
  ...
  //
  int i = 1;
  for (core::ConnectedComponents_t::const_iterator itcc = roadmap()->connectedComponents().begin();
        ...
        if (!pathValid || !belongs(q_new, newNodes)) {
          ...
          if (!pathValid) {
            ...
            if (path) {
              ...
              if (paraPathValid) {  // only add if the full path is valid, otherwise it's the same as the straight line
                                    // (because we can't extract a subpath of a parabola path)
  ...
  for (DelayedEdges_t::const_iterator itEdge = delayedEdges.begin(); itEdge != delayedEdges.end(); ++itEdge) {
  ...
  //
  for (core::Nodes_t::const_iterator itn1 = newNodes.begin(); itn1 != newNodes.end(); ++itn1) {
    for (core::Nodes_t::const_iterator itn2 = boost::next(itn1); itn2 != newNodes.end(); ++itn2) {
...
// This method call SteeringMethodParabola, but we don't try to connect two confuration, instead we shoot a random
// alpha0 and V0 valide for the initiale configuration and then compute the final point. Then we check for collision
// (for the trunk)  and we check if the final point is in a valide configuration (trunk not in collision but limbs in
// accessible contact zone). (Not anymore ) If this is true we do a reverse collision check until we find the first
// valide configuration, then we check for the friction cone and impact velocity constraint.(Not anymore : can't find
// normal after this)
core::PathPtr_t ParabolaPlanner::computeRandomParabola(core::NodePtr_t x_start, core::ConfigurationPtr_t q_target,
                                                       DelayedEdges_t& delayedEdges) {
  ...
  if ((!valid) && validPath->timeRange().second > path->timeRange().first) {
    ...
    if (contactValid) {
      ...
      fcl::Vec3f normal(-boost::dynamic_pointer_cast<core::CollisionValidationReport>(report->configurationReport)
                             ->result.getContact(0)
                             .normal);
      hppDout(notice, "normal = " << normal);
      // fill extraDof with normal :
      ...
      const value_type Vimp =
          sqrt(1 + (-9.81 * X * inv_x_th_dot_0_sq + tan(alpha0)) * (-9.81 * X * inv_x_th_dot_0_sq + tan(alpha0))) *
      ...
      bool fail3 = smParabola_->third_constraint(0, X_theta, Z, alpha_imp_min, alpha_imp_max, &alpha_imp_sup,
...
void ParabolaPlanner::tryDirectPath() {
  ...
  for (core::Nodes_t::const_iterator itn = roadmap()->goalNodes().begin(); itn != roadmap()->goalNodes().end();
       ++itn) {
...
void ParabolaPlanner::configurationShooter(const core::ConfigurationShooterPtr_t& shooter) {
...
/* core::PathVectorPtr_t DynamicPlanner::solve (){
  ...
  core::NodePtr_t initNode = roadmap ()->initNode();
  for (core::Nodes_t::const_iterator itn = roadmap ()->goalNodes ().begin();itn != roadmap ()->goalNodes ().end ();
++itn) { core::ConfigurationPtr_t q1 ((initNode)->configuration ()); core::ConfigurationPtr_t q2 ((*itn)->configuration
()); assert (*q1 != *q2); hppStartBenchmark(EXTEND); path = extend(initNode,q2); hppStopBenchmark (EXTEND);
    hppDisplayBenchmark (EXTEND);
    if (path) {
      ...
      if (pathValid ) {
        ...
        if (path) {
          ...
          bool paraPathValid = rbprmPathValidation->validate (path, false, validPath, report, filter);
          if (paraPathValid) { // only add if the full path is valid, otherwise it's the same as the straight line
(because we can't extract a subpath of a parabola path) hppDout(notice, "#### parabola path valid !");
            core::ConfigurationPtr_t q_last (new core::Configuration_t(validPath->end ()));
            core::NodePtr_t x_last = rbprmRoadmap()->addNode(q_last);
            rbprmRoadmap()->addEdge(x_new,x_last,validPath);
            pathVector->appendPath (validPath);
...
void ParabolaPlanner::computeGIWC(const core::RbprmNodePtr_t node, core::ValidationReportPtr_t report) {
  ...
  */
    ...
    if (tangent0.dot(tangent0) < 0.001) tangent0 = normal.cross(polytope::vector3_t(0, 1, 0));

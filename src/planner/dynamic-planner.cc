...
DynamicPlannerPtr_t DynamicPlanner::createWithRoadmap(core::ProblemConstPtr_t problem, const RoadmapPtr_t& roadmap) {
...
DynamicPlanner::DynamicPlanner(core::ProblemConstPtr_t problem)
    : BiRRTPlanner(problem),
      ...
      roadmap_(
          std::dynamic_pointer_cast<core::Roadmap>(core::RbprmRoadmap::create(problem->distance(), problem->robot()))),
...
DynamicPlanner::DynamicPlanner(core::ProblemConstPtr_t problem, const RoadmapPtr_t& roadmap)
...
bool DynamicPlanner::tryParabolaPath(const core::NodePtr_t& x_near, core::ConfigurationPtr_t q_last,
                                     const core::ConfigurationPtr_t& target, bool reverse, core::NodePtr_t& x_jump,
                                     core::NodePtr_t& x_new, core::PathPtr_t& kinoPath, core::PathPtr_t& paraPath) {
  bool success(false);
  core::PathValidationPtr_t pathValidation(problem()->pathValidation());
  core::PathPtr_t validPath;
  core::PathValidationReportPtr_t report;
  const size_type indexECS =
      problem()->robot()->configSize() - problem()->robot()->extraConfigSpace().dimension();  // ecs index
  ...
  if (paraPath) {
    ...
    if (paraPath->length() >
        0) {  // only add if the full path is valid (because we can't extract a subpath of a parabola path)
      hppDout(notice, "!! parabola path valid !");
      ParabolaPathPtr_t parabolaPath = std::dynamic_pointer_cast<ParabolaPath>(paraPath);
...
void DynamicPlanner::oneStep() {
  ...
  for (std::vector<core::ConnectedComponentPtr_t>::const_iterator itcc = endComponents_.begin();
    ...
    if (path) {
      ...
      if (pathValidFromStart && pathValidFromEnd)  // qrand was successfully connected to both trees
      {
        ...
        if (t_final != path->timeRange().first) {
          ...
          if (startComponentConnected) {  // now try to connect both nodes (qnew -> qnewEnd)
            hppStartBenchmark(EXTEND);
...
void DynamicPlanner::computeGIWC(const core::NodePtr_t x, bool use_bestReport) {
  ...
  rbprmPathValidation_->getValidator()
      ->randomnizeCollisionPairs();  // FIXME : remove if we compute all collision pairs
  rbprmPathValidation_->getValidator()->computeAllContacts(true);
  hppDout(notice,
          "Compute GIWC, call validate for configuration : " << pinocchio::displayConfig(*(x->configuration())));
  ...
  if (use_bestReport) {
    ...
    node->chooseBestContactSurface(report, std::dynamic_pointer_cast<pinocchio::RbPrmDevice>(problem()->robot()));
  }
...
void DynamicPlanner::tryConnectInitAndGoals() {
  ...
  for (core::NodeVector_t::const_iterator itn = roadmap()->goalNodes().begin(); itn != roadmap()->goalNodes().end();
       ++itn) {
    ...
    if (projPath) {
      core::PathValidationReportPtr_t report;
      // roadmap ()->addEdge (initNode, *itn, projPath);  // (TODO a supprimer)display the path no matter if it's
      // successful or not

      bool pathValid = pathValidation->validate(projPath, false, validPath, report);

      // roadmap ()->addEdge (initNode, *itn, validPath);  // (TODO a supprimer)display the path no matter if it's
      // successful or not
      ...
      if (pathValid && validPath->timeRange().second !=
        ...
        if (tryJump_) {
          ...
          bool trunkValid = rbprmPathValidation->getValidator()->validate(
              (*projPath)(report->parameter, successPathOperator), valReport, filter);
          if (trunkValid && successPathOperator) {  // if it failed because of the ROM, we can try a parabola
            ...
            hppDout(notice, "trunk in collision");
                   ...
                   path->timeRange().first) {  // add the last valid configu reached to the roadmap
...
Problem::declareParameter(core::ParameterDescription(
    ...
    "The lenght of the feet along X axis (assuming rectangular feet).", core::Parameter(0.)));
Problem::declareParameter(core::ParameterDescription(
    core::Parameter::FLOAT, "DynamicPlanner/sizeFootY",
    "The lenght of the feet along Y axis (assuming rectangular feet).", core::Parameter(0.)));
Problem::declareParameter(core::ParameterDescription(
    core::Parameter::FLOAT, "DynamicPlanner/friction",
    "Value of the friction coefficient between the feet of the robot and the ground.", core::Parameter(0.5)));
Problem::declareParameter(core::ParameterDescription(core::Parameter::BOOL, "DynamicPlanner/tryJump",

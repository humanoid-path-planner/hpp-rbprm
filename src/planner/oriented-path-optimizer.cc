...
/**
 * @brief OrientedPathOptimizer::checkReplaceOrientation recursively check if an element of the path vector can be
 * changed to an oriented path : the final state of the previous element and the initial state of the next element must
 ...
 * @return
 */
bool OrientedPathOptimizer::checkReplaceOrientation(const size_t index, const size_t lastIndex,
  ...
  if (index < lastIndex - 1) {       // test if next element can be adjusted
    if (orientedValid[index + 1]) {  // make a recursive test for the next elements
      ...
      replaceValid[index] =
          checkReplaceOrientation(index + 1, lastIndex, replaceValid, orientedValid, orientedPaths, resultPaths);
...
core::PathPtr_t OrientedPathOptimizer::steer(ConfigurationIn_t q1, ConfigurationIn_t q2) const {

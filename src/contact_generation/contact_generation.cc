...
void maintain_contacts_combinatorial_rec(const hpp::rbprm::State& currentState, const std::size_t depth,
  ...
  while (!contactOrder.empty() && size != i) {
    ...
    // if(!
    //((std::find(fixed.begin(), fixed.end(),std::string("hrp2_rleg_rom")) == fixed.end() && contactRemoved ==
    // std::string("hrp2_lleg_rom")) || (std::find(fixed.begin(), fixed.end(),std::string("hrp2_lleg_rom")) ==
    // fixed.end() && contactRemoved == std::string("hrp2_rleg_rom"))))
    {
...
Q_State flatten(const T_DepthState& depthstates) {
  ...
  for (T_DepthState::const_iterator cit = depthstates.begin(); cit != depthstates.end(); ++cit) {
    for (CIT_State ccit = cit->begin(); ccit != cit->end(); ++ccit) res.push(*ccit);
  }
  return res;
}
...
Q_State maintain_contacts_combinatorial(const hpp::rbprm::State& currentState, const std::size_t maxBrokenContacts) {
  ...
  if (currentState.nbContacts == 1) {
    ...
    hppDout(warning,
            "Maintain_contacts_combinatorial called with a state with only 1 contact. FIXME, why does it happen ? ");
  } else {
...
bool maintain_contacts_stability_rec(hpp::rbprm::RbPrmFullBodyPtr_t fullBody,
  ...
  if (!candidates.empty()) {
    ...
    if (maintain_contacts_stability_rec(fullBody, targetRootConfiguration, copy_candidates, contactLength,
...
std::vector<pinocchio::CollisionObjectPtr_t> getAffObjectsForLimb(
  ...
  for (std::map<std::string, std::vector<std::string> >::const_iterator fIt = affFilters.begin();
  ...
  if (!settingFound) {
    ...
    std::cout << "No affordance filter setting found for limb " << limb << ". Has such filter been set?" << std::endl;
    // Use all AFF OBJECTS as default if no filter setting exists
    for (std::map<std::string, std::vector<std::pair<std::string, pinocchio::CollisionObjectPtr_t> > >::const_iterator
             affordanceIt = affordances.map.begin();
    ...
    for (std::vector<std::string>::const_iterator affTypeIt = affTypes.begin(); affTypeIt != affTypes.end();
         ++affTypeIt) {
  ...
  if (affs.empty()) throw std::runtime_error("No aff objects found for limb " + limb);
  return affs;
}
...
ProjectionReport maintain_contacts_stability(ContactGenHelper& contactGenHelper, ProjectionReport& currentRep) {
  ...
  maintain_contacts_stability_rec(contactGenHelper.fullBody_, contactGenHelper.workingState_.configuration_,
                                  contactGenHelper.candidates_, contactLength, contactGenHelper.acceleration_,
...
std::vector<std::string> extractEffectorsName(const rbprm::T_Limb& limbs) {
  ...
  for (rbprm::T_Limb::const_iterator cit = limbs.begin(); cit != limbs.end(); ++cit) res.push_back(cit->first);
  return res;
}
...
std::vector<std::string> sortLimbs(const State& currentState, const std::vector<std::string>& freeLimbs) {
  ...
  while (!order.empty()) {
    ...
    order.pop();
    if (std::find(freeLimbs.begin(), freeLimbs.end(), l) != freeLimbs.end()) tools::insertIfNew(res1, l);
  }
...
ProjectionReport genColFree(ContactGenHelper& contactGenHelper, ProjectionReport& currentRep) {
  ...
  for (std::vector<std::string>::const_iterator cit = freeLimbs.begin(); cit != freeLimbs.end() && res.success_;
       ++cit) {
    res = projection::setCollisionFree(contactGenHelper.fullBody_,
  ...
  std::vector<std::string> effNotContactingNames(
      extractEffectorsName(contactGenHelper.fullBody_->GetNonContactingLimbs()));
  for (std::vector<std::string>::const_iterator cit = effNotContactingNames.begin();
...
void stringCombinatorialRec(std::vector<std::vector<std::string> >& res, const std::vector<std::string>& candidates,
                            const std::size_t depth) {
  if (depth == 0) return;
  std::vector<std::vector<std::string> > newStates;
  for (std::vector<std::vector<std::string> >::iterator it = res.begin(); it != res.end(); ++it) {
    for (std::vector<std::string>::const_iterator canditates_it = candidates.begin();
...
std::vector<std::vector<std::string> > stringCombinatorial(const std::vector<std::string>& candidates,
...
void gen_contacts_combinatorial_rec(const std::vector<std::string>& freeEffectors, const State& previous,
  ...
  for (std::vector<std::vector<std::string> >::const_iterator cit = allNewStates.begin(); cit != allNewStates.end();
       ++cit) {
...
ProjectionReport maintain_contacts(ContactGenHelper& contactGenHelper) {
  ...
  if (contactGenHelper.candidates_.empty() && !contactGenHelper.workingState_.contacts_.empty()) {
    ...
    contactGenHelper.candidates_ =
        maintain_contacts_combinatorial(contactGenHelper.workingState_, contactGenHelper.maxContactBreaks_);
  ...
  while (!candidates.empty() && !rep.success_) {
    ...
    rep =
        projectToRootConfiguration(contactGenHelper.fullBody_, contactGenHelper.workingState_.configuration_, cState);
    hppDout(notice, "maintain contacts, projection success : " << rep.success_ << " for contacts : ");
    for (std::map<std::string, bool>::const_iterator cit = cState.contacts_.begin(); cit != cState.contacts_.end();
         ++cit) {
    ...
    if (rep.success_) {
      ...
      rep.success_ =
          contactGenHelper.fullBody_->GetCollisionValidation()->validate(rep.result_.configuration_, valRep);
      hppDout(notice, "maintain contact collision for config : r(["
    ...
    if (rep.success_) {
      if (contactGenHelper.quasiStatic_ && contactGenHelper.testReachability_) {
        reachability::Result resReachability =
            reachability::isReachable(contactGenHelper.fullBody_, contactGenHelper.workingState_, rep.result_);
...
sampling::T_OctreeReport CollideOctree(const ContactGenHelper& contactGenHelper, const std::string& limbName,
  ...
  std::vector<pinocchio::CollisionObjectPtr_t> affordances =
      getAffObjectsForLimb(limbName, contactGenHelper.affordances_, contactGenHelper.affFilters_);
...
hpp::rbprm::State findValidCandidate(const ContactGenHelper& contactGenHelper, const std::string& limbId,
                                     RbPrmLimbPtr_t limb, core::CollisionValidationPtr_t validation,
                                     bool& found_sample, bool& found_stable, bool& unstableContact,
                                     const sampling::HeuristicParam& params, const sampling::heuristic evaluate = 0) {
  ...
  for (; !found_sample && it != finalSet.end(); ++it) {
    ...
    /*ProjectionReport */ rep = projectSampleToObstacle(contactGenHelper.fullBody_, limbId, limb, bestReport,
                                                        validation, configuration, current);
    ...
    if (rep.success_) {
      ...
      if (!contactGenHelper.checkStabilityGenerate_ ||
          (rep.result_.nbContacts == 1 &&
           !contactGenHelper.stableForOneContact_)) {  // only check projection, success !
        ...
        if (robustness >= contactGenHelper.robustnessTreshold_) {
          ...
          if (contactGenHelper.testReachability_) {
            ...
            if (contactGenHelper.quasiStatic_) {
              ...
              // resReachability = reachability::isReachable(contactGenHelper.fullBody_,previous,rep.result_); // TODO
              // : use a parameter to choose between both cases
            } else {
              hppStartBenchmark(REACHABILITY_CONTACT);
              resReachability = reachability::isReachableDynamic(
                  contactGenHelper.fullBody_, previous, rep.result_, contactGenHelper.tryQuasiStatic_,
                  std::vector<double>(), contactGenHelper.reachabilityPointPerPhases_);
          ...
          if (isReachable) {  // reachable
            ...
            if (!found_stable) {
              ...
              // if no reachable state are found, we keep the first stable configuration found (ie. the one with the
            ...
            /*   // DEBUGING PURPOSE : call again evaluate on the sample found
              ...
              Eigen::Vector3d eDir(contactGenHelper.direction_[0], contactGenHelper.direction_[1],
              contactGenHelper.direction_[2]); // remove eDir.normalize();       // remove hppDout(notice,"sample =
              "<<(*(bestReport.sample_)).startRank_); // remove sampling::heuristic eval =  evaluate == 0 ?
              limb->evaluate_ : evaluate;// remove
  ...
  /*
      std::ofstream fout;
      fout.open("/local/fernbac/bench_iros18/bench_contactGeneration/candidates.log", std::fstream::out |
     std::fstream::app); fout<<"evaluatedCandidates "<<evaluatedCandidates<<std::endl; fout.close();
  */
  if (contactGenHelper.fullBody_->usePosturalTaskContactCreation() && !found_sample) {
    // as the contact generator is not complete when usePosturalTaskContactCreation==True, if it fail we retry without
    // this option
    contactGenHelper.fullBody_->usePosturalTaskContactCreation(false);
    current = findValidCandidate(contactGenHelper, limbId, limb, validation, found_sample, found_stable,
...
ProjectionReport generate_contact(const ContactGenHelper& contactGenHelper, const std::string& limbName,
                                  sampling::HeuristicParam& params, const sampling::heuristic evaluate) {
  ProjectionReport rep;
  ...
  rep.result_ = findValidCandidate(contactGenHelper, limbName, limb, validation, found_sample, found_stable,
                                   unstableContact, params, evaluate);
...
ProjectionReport gen_contacts(ContactGenHelper& contactGenHelper) {
  ...
  hppDout(notice, "working state config : r(["
                      << pinocchio::displayConfig(contactGenHelper.workingState_.configuration_) << "])");
  ...
  while (!candidates.empty() && !rep.success_) {
    ...
    if (cState.second.empty() && checkStability &&
        (contactGenHelper.workingState_.nbContacts >= 2 || contactGenHelper.stableForOneContact_)) {
      hppDout(notice,
              "List of free limbs empty in gen_contact, check stability for workingState with contact maintained");
      double robustness = stability::IsStable(contactGenHelper.fullBody_, contactGenHelper.workingState_,
    ...
    for (std::vector<std::string>::const_iterator cit = cState.second.begin(); cit != cState.second.end(); ++cit) {
      ...
      params.comSpeed_ = fcl::Vec3f(cState.first.configuration_[cfgSize - 6], cState.first.configuration_[cfgSize - 5],
      ...
      params.tfWorldRoot_.setTranslation(
          fcl::Vec3f(cState.first.configuration_[0], cState.first.configuration_[1], cState.first.configuration_[2]));
      params.tfWorldRoot_.setQuatRotation(
          fcl::Quaternion3f(cState.first.configuration_[6], cState.first.configuration_[3],
                            cState.first.configuration_[4], cState.first.configuration_[5]));

      if (cit + 1 == cState.second.end()) contactGenHelper.checkStabilityGenerate_ = checkStability;
...
projection::ProjectionReport repositionContacts(ContactGenHelper& helper) {
  ...
  while (!result.stable && !oldOrder.empty()) {
    ...
    bool notFound(true);
    for (std::vector<std::string>::const_iterator cit = group.begin(); notFound && cit != group.end(); ++cit) {
      ...
      params.comSpeed_ = fcl::Vec3f(helper.workingState_.configuration_[cfgSize - 6],
      ...
      params.sampleLimbName_ = *cit;
      params.tfWorldRoot_ = fcl::Transform3f();
      params.tfWorldRoot_.setTranslation(fcl::Vec3f(helper.workingState_.configuration_[0],
                                                    helper.workingState_.configuration_[1],
                                                    helper.workingState_.configuration_[2]));
      params.tfWorldRoot_.setQuatRotation(
          fcl::Quaternion3f(helper.workingState_.configuration_[6], helper.workingState_.configuration_[3],
                            helper.workingState_.configuration_[4], helper.workingState_.configuration_[5]));
